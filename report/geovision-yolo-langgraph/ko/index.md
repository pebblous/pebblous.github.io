---
title: 위성이 보고, AI가 읽는다
subtitle: YOLO26 + LangGraph 에이전틱 지구관측의 탄생
date: 2026-04-15
category: report
source: index.html
...
_YOLO26 + LangGraph 에이전틱 지구관측의 탄생_
...
![DOTA 위성영상 데이터셋 18개 클래스 샘플: 대형 차량, 수영장, 헬리콥터, 창고, 선박, 축구장, 야구장, 공항, 원형 교차로 등 항공/위성 이미지에서 검출해야 하는 실제 객체들](./image/img-05-dota-classes-grid.avif)
*▲ DOTA-v2.0 데이터셋의 18개 클래스 실제 위성영상 샘플 — 차량, 선박, 항공기, 야구장, 공항 등 1,793,658개 인스턴스를 포함한다 | Source: [Ultralytics DOTA-v2.0 Docs](https://docs.ultralytics.com/datasets/obb/dota-v2/)*
...
![YOLO26 아키텍처 파이프라인: 입력 전처리 → 백본 피처 추출 → 다중 스케일 피처 맵 → 경량 퓨전 넥 → NMS-Free 예측 → STAL 라벨 할당 → MuSGD 옵티마이저 → 양자화 배포](./image/img-01-yolo26-architecture.png)
*▲ YOLO26 전체 아키텍처 파이프라인 — NMS-Free 추론, STAL 소형 타겟 라벨 할당, MuSGD 옵티마이저가 핵심 혁신이다 | Source: [Sapkota et al., arXiv:2509.25164](https://arxiv.org/abs/2509.25164)*
...
![YOLO26 4대 혁신: (a) Distribution Focal Loss 제거, (b) End-to-End NMS-Free 추론, (c) ProgLoss 점진적 손실 균형 + STAL 소형 타겟 라벨 할당, (d) MuSGD 안정 수렴 옵티마이저](./image/img-02-yolo26-innovations.png)
*▲ YOLO26의 4대 아키텍처 혁신 — (a) DFL 제거로 검출 헤드 단순화, (b) NMS-Free end-to-end 추론, (c) ProgLoss+STAL 소형 객체 강화, (d) MuSGD 안정 수렴 | Source: [Sapkota et al., arXiv:2509.25164](https://arxiv.org/abs/2509.25164)*
...
![DOTA 데이터셋 항구 위성영상에서의 회전 바운딩 박스(OBB) 객체 검출 결과: 선박(초록), 차량(보라) 등 다중 클래스를 방향 정보와 함께 검출](./image/img-06-dota-obb-detection.avif)
*▲ 항구 위성영상에서의 회전 바운딩 박스(OBB) 검출 — 선박, 차량 등을 방향 정보와 함께 정밀 검출한다. 위성영상의 객체는 임의 방향으로 회전하므로 수평 박스(HBB)로는 정확한 경계를 잡을 수 없다 | Source: [Ultralytics DOTA-v2.0](https://docs.ultralytics.com/datasets/obb/dota-v2/)*
...
![Ultralytics YOLO 모델 성능 비교 차트: COCO mAP50-95 vs T4 TensorRT FP16 레이턴시. YOLO26이 모든 모델 크기에서 파레토 프론티어를 갱신](./image/img-03-yolo-performance.png)
*▲ YOLO 계열 전체 성능 비교 — YOLO26(파란 선)이 모든 모델 크기에서 정확도-속도 파레토 프론티어를 갱신했다 | Source: [Ultralytics](https://github.com/ultralytics/assets)*
...
![YOLO26 vs RT-DETR/DEIM 성능 비교: COCO mAP50-95 대 T4 TensorRT FP16 레이턴시. YOLO26이 모든 크기에서 RT-DETR 계열을 능가](./image/img-04-yolo26-vs-rtdetr.png)
*▲ YOLO26 vs Transformer 기반 검출기(RT-DETR, DEIM) 성능 비교 — YOLO26이 동일 레이턴시에서 일관되게 더 높은 mAP를 달성한다 | Source: [Sapkota et al., arXiv:2509.25164](https://arxiv.org/abs/2509.25164)*
