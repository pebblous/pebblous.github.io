...
- Always use `/scripts/common-utils.js` — never `/js/article-page.js` or `/js/common-utils.js` (둘 다 deprecated, 파일 미존재)
- Always use `PebblousPage.init({ mainTitle, subtitle, ... })` — never `PebblousBlog.init(config)` (존재하지 않는 함수)
...
**⛔ EN 번역 작성도 예외 없음**: "번역"이라도 새 글 작성과 동일한 Pre-Write Read + 금지 패턴 점검을 실행한다. KO HTML을 복사해서 번역하는 과정에서 deprecated 경로·비표준 패턴이 그대로 유입되는 사고가 반복됐다 (사례: `report/claude-code-workflows-enterprise-ai/en/` — NanoClaw가 `/js/common-utils.js` + `PebblousBlog.init()` 그대로 복사 → 페이지 전체 렌더링 불가, PR #269).
...
grep -n "/js/common-utils.js" [html파일]     # → 0줄이어야 함 (deprecated 경로)
grep -n "PebblousBlog.init" [html파일]       # → 0줄이어야 함 (존재하지 않는 함수)
grep -n "PebblousPage.init" [html파일]       # → 1줄이어야 함
```
