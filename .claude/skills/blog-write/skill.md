...
| `mainTitle` | **신문 1면 헤드라인** — 정본 `docs/ko-style-standard.md` §4-2 (2026-09-11). ① 주어(무엇·누가)를 맨 앞에 ② 결론을 그대로(반전·수수께끼·비유·관형절 두 겹 금지) ③ 평이한 낱말. 명사구·서술 종결 어느 쪽이든. 허용: 질문형 · "— 출처" 꼬리 · "X: Y"(X=짧은 이름표) · 낱말 하나 '강조' | 20~35자 (45자 초과 위반) |
...
3. **검증 체크**:
   ...
   - [ ] 세 슬롯 모두 따옴표 인용·발화 인용+반전·대조공식("A가 아니라 B")·미끼·키워드 나열·잘린 명사형 없음. 줄표는 "— 출처/기관/수치" 꼬리만, 콜론은 "X: Y"(X=12자 이하 이름표)만 (`ko-style-standard.md` §4-2)
   - [ ] `mainTitle`이 결론을 그대로 말하는가 — "~쪽은 …였다"·"무엇을 …"로 감추거나 비유로 돌리지 않았는가
