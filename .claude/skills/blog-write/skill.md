...
제목 정본 = `docs/ko-style-standard.md` §4-2 v3 (2026-09-13 형님 판정) — 먼저 Read. 슬롯·SEO·OG 규격은 `docs/title-strategy.md`. 아래는 이 스킬이 쓰는 만큼만 옮긴 요약이고, 문안이 갈리면 정본을 따른다.

1. **리서치 초안 확인**: `_workspace/01_research.md`의 `추천 제목` 섹션에서 mainTitle 후보, subtitle 후보, hook(제목에서 뺀 기관·수치·방법), pageTitle 키워드 소재, 검색 의도를 확인한다. 리서치 후보는 방향일 뿐이다. 제목은 본문을 다 쓴 뒤 아래 절차로 새로 짓는다.

2. **mainTitle 작성 절차** (본문을 다 쓴 뒤):
   ① **중학생에게 이 글을 한 문장으로** — 본문을 다 쓴 뒤 이 문장을 먼저 적는다. 제목이 아니라 글의 요점이다.
   ② 그 문장에서 **세 형태로 후보 3개**를 짓는다.
      - 질문형(독자에게 묻는다. 두 갈래를 맞세우면 더 좋다): "AI는 생각의 도구일까, ‘인지 바이러스’일까?" · "불량을 한 번도 안 보고, 불량을 잡을 수 있을까?"
      - 신문 명사형(주어 뒤 쉼표, 명사로 끝난다): "SNS 유해 글 단속, 열에 여덟은 딱지 없이 통과" · "공장 검사 AI, 불량 목록 대신 '정상 모습'을 학습 중"
      - 현재형 주장(지금도 참인 한 문장. ~한다·~이다·~없다): "AI는 신입 일자리를 가장 먼저 없앤다 — 스탠퍼드 연구" · "지난 실험 기록이 교과서보다 AI를 더 잘 가르친다"
   ③ **두 시험**을 스스로 걸어 둘 다 통과하는 것을 고른다.
      - 중학생 시험: 제목만 읽은 중학생이 "이 기사는 ~에 관한 것"이라고 한 줄로 말할 수 있는가
      - 클릭 시험: 눌러 보고 싶은가. 궁금증·놀라움·'내 일 같음' 중 하나는 있어야 한다
   ④ 제목에서 뺀 **기관·수치·방법·출처는 subtitle로 내린다**(40~70자, 신문체, 명사 종결 허용). 제목이 쉬워진 만큼 부제가 정확해야 한다. **부제도 중학생 시험을 통과해야 한다**(2026-09-18): 부제만 읽은 중학생이 무슨 일이 있었는지 말할 수 있어야 한다. 낯선 업계 용어는 부제에서도 쉬운 말로 푼다(거버넌스 → 관리). 온전한 문장으로 — 길이 맞추려 낱말을 빼서 문장을 부수지 않는다, 넘치면 수치·출처를 덜어 낸다. 수치에는 무엇의 수치인지 붙인다. 실패 예 "클라우데라 에너지·유틸리티 조사에서 65%가 거의 전부 거버넌스라 답했고, 기준을 전부로 좁히면 20% 밑이다" → "클라우데라 조사 — 에너지·유틸리티 기업 65%는 데이터 '거의 전부'를 관리한다고 답했지만, '전부'로 물으면 20%도 안 된다"

   금지(이름으로):
   - **과거형 서술 종결**(~했다·~였다·~았다/었다) — 사건 보고문이 된다. 있으면 무조건 다시 짓는다. 실패 예 "…적중 27.7%를 찾았다" · "…83%는 방법이 그대로였다"
   - **제목 안 수치** — 중학생은 수치로 기사를 못 알아본다. 수치는 subtitle로. 실패 예 "14일에서 2.5일로" · "1,134건". 한글 수사("열에 여덟")는 수치가 아니다
   - **낯선 고유명사·전문용어** — 회사명·모델명은 subtitle로, 업계 용어는 subtitle에서도 쉬운 말로 푼다. AI·SNS·로봇처럼 중학생이 아는 말만. 실패 예 "루마필드 CT 검사 AI" · "에이전트 메모리" · "모더레이션"
   - **AI 어투** — 사물 주어 + "~가 붙었다/~가 갈렸다", 번역투. 실패 예 "증거 등급이 하나씩 붙었다"
   - **순우리말 풀이 술어** — "~재다"는 "~측정하다"로. 한자어 술어(측정·검증·비교·분석)가 축약적이고 권위 있다. 부제도 같다. 실패 예 "발사 전에 재 둔 PLATO…" · "실력을 재는 잣대는 뒤처진다" · 부제 "…갈라 쟀다"
   - 유지하는 금지 — 미끼("충격"·"N가지 비밀") · 키워드 나열 · 잘린 명사형("~이후"·"~전략") · 발화 인용+반전 · "A가 아니라 B" 대조 공식

3. **3슬롯 최종 결정**:

| 슬롯 | 기준 | 글자 수 |
|------|------|---------|
| `pageTitle` | mainTitle의 **검색 변형** — 핵심 키워드 앞배치 + 구체적 약속 + ` \| 페블러스`. 검색 변형이므로 수치·기관명이 들어가도 된다(§4-2 v3 의 수치 금지는 mainTitle 에만) | 50-60자 |
| `mainTitle` | **중학생이 알아듣고 누르고 싶은 제목** — 위 절차로 지은 세 형태(질문형·신문 명사형·현재형 주장) 중 하나. 주어(무엇·누가) 먼저, 평이한 낱말. 허용: "— 출처/기관" 꼬리(낱말 몇 개, 문장 아님) · "X: Y"(X=짧은 이름표) · 낱말 하나 '강조' | 20~35자 (45자 초과 위반) |
| `subtitle` | **정확한 헤드라인 자리** — 기관·수치·방법·출처를 신문체로, **중학생이 읽어 무슨 일인지 알 수 있는 온전한 문장으로**(낯선 용어는 풀고, 길이 맞추려 문장을 부수지 않는다). mainTitle에 없는 보조 키워드 포함. 예: 제목 "SNS 유해 글 단속, 열에 여덟은 딱지 없이 통과" → 부제 "블루스카이 2025년 라벨 1,060만 건 첫 외부 감사 — 정밀도 0.837, 재현율 0.222" | 40~70자 |

4. **검증 체크**:
   - [ ] `mainTitle`이 과거형 서술 종결이 아닌가 — 있으면 다시 짓는다
   - [ ] `mainTitle` 안에 아라비아 숫자 수치가 없는가 — 있으면 subtitle로 내린다
   - [ ] `mainTitle`·`subtitle`에 낯선 고유명사·전문용어·AI 어투·재다류(→측정하다)가 없는가
   - [ ] **`mainTitle`에 도메인 주어(무엇에 관한 글인지)가 있는가** — 대조·훅·여운이 주어를 대체하면 안 된다 (title-strategy §3.1 "주어 실종 훅" 금지). ❌"며칠은 이기고 몇 년은 지는 이유"
   - [ ] 세 슬롯 모두 따옴표 인용·발화 인용+반전·대조공식("A가 아니라 B")·미끼·키워드 나열·잘린 명사형 없음. 줄표는 "— 출처/기관" 꼬리만, 콜론은 "X: Y"(X=12자 이하 이름표)만 (§4-2 유지 금지)
   - [ ] `pageTitle`은 mainTitle의 검색 변형인가 — **같은 사실**, 검색자의 언어. 완전 동일도, 억지 신규 문장 발명도 금지 (title-strategy §0)
   - [ ] `pageTitle` 앞쪽 1/3에 핵심 키워드
   - [ ] `subtitle`이 mainTitle이 비운 자리(기관·수치·방법·출처)를 채우는가 — mainTitle 동격 재진술 아님
   - [ ] `subtitle`만 읽은 중학생이 무슨 일이 있었는지 말할 수 있는가 — 낯선 용어(거버넌스 등)를 풀었는가, 길이 맞추려 부순 문장이 아닌가, 수치에 무엇의 수치인지 붙었는가
   - [ ] `mainTitle` 소리 내어 읽었을 때 — 신문 1면에 실릴 문장인가. 두 시험(중학생·클릭)을 다시 건다. 검사기(`tools/title-census.py`)와 발행 엔진 `title-gate` 눈감은 심판이 뒤에서 다시 재지만, **검사기 통과 ≠ 좋은 제목**이다
   - [ ] EN 제목은 한국어 직역이 아닌 영어 검색 의도에 맞게 재작성

5. **EN 제목** (bilingual 시): 한국어 제목을 번역하지 말고, 영어 검색 의도에 맞게 별도 작성. Title Case 적용. 두 시험(중학생·클릭)과 과거형 종결·수치·낯선 고유명사 금지는 EN에도 같이 건다.

## PebblousPage.init() — 필수 설정

```javascript
PebblousPage.init({
  mainTitle: "메인 제목",          // REQUIRED — h1에 렌더링 (§4-2 v3 세 형태 중 하나, 수치·과거형 종결 없음)
  subtitle: "부제목",              // REQUIRED — h1 아래 (정확한 헤드라인 자리 — 기관·수치·방법 + 보조 키워드)
...
- 따옴표 인용·대조공식·인용+반전·미끼 금지 (`ko-style-standard.md` §4-2 유지 금지가 description에도 적용)
