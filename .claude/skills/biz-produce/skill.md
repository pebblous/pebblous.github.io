...
description: >
  ...
  기획 → 병렬 3트랙 리서치(재무/제품/시장) → 합성 → 이중언어 HTML 작성(KO+EN 병렬) → 퍼블리싱을
  멀티 에이전트로 일괄 실행.
  "비즈 인사이트 써줘", "BizReport 작성해줘", "기업 분석 해줘",
  "biz-produce 실행", "비즈 리포트 [기업명]" 요청 시 이 스킬을 반드시 사용.
---

# biz-produce: BizReport 기업 분석 파이프라인 오케스트레이터
...
| validate-articles.js 오류 | HTML 구조 문제 | 오류 내용 확인 후 biz-writer-ko/en에 수정 지시 |
