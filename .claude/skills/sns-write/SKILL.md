...
description: >
  ...
  Default: short magazine-cover essay (voice=sns-cover).
  Optional: long reflective essay (voice=reflective) for Facebook 장문 / LinkedIn 장문 / Medium 마스터 카피.
...
| 호출 시 voice | 출처 | 사용 시점 |
|--------------|------|---------|
| `sns-cover` (기본) | `.claude/skills/voice-edit/voices/sns-cover.md` | LinkedIn/Twitter/Facebook 짧은 카피 |
| `reflective` | `.claude/skills/voice-edit/voices/reflective.md` | 장문 에세이 슬롯 (Facebook 장문, LinkedIn 장문, Medium 마스터 카피) |
...
| 입력 | 적용 voice |
|------|-----------|
| platform=`linkedin\|twitter\|facebook\|all` (기본) | `sns-cover` |
| platform=`essay` | `reflective` |
| `--voice=sns-cover` 명시 | `sns-cover` |
| `--voice=reflective` 명시 | `reflective` |
...
- **Facebook (KO/EN)**: 구체적 숫자/사례로 시작, 3-4문단 확장 서술

`reflective` voice (명시 호출 시):
...
platform=`all`은 sns-cover 4채널만 (essay는 명시 호출 시에만).
