...
JH가 없는 무인 모드이므로 5-B/5-C/5-D/5-E를 자의적 판단으로 skip하는 사고가 자주 났다 (예: ICLR run에서 "본문 충분히 풍부함"으로 5-B/5-C 둘 다 skip → image-reinforce/bibliography 누락). 베이스 스킬의 객관 skip 기준을 그대로 따르되 express에서는 다음을 추가:
...
| 5-E ko-prose-humanizer | **항상 호출 (KO + EN 양쪽)**. text-reinforce(5-B)가 내용 보강을 담당하고, ko-prose-humanizer(5-E)가 AI 문체 진단·교정(T1~T11)을 담당한다. 둘은 역할이 다르므로 중복이 아님. em-dash 비율 기준: 1/500자 초과 시 수술적 교정. |

→ logger note에 skip 사유를 적을 때 "충분히 풍부하다 / 이미 잘 돼있다" 같은 주관어 사용 금지. 객관 조건 (`references<4`, `images>=4 with topic match`) 만 인용.

### Phase 7-B 분기 — sns-write 자동 실행

표준 스킬의 Phase 7-B(sns-write)를 **그대로 실행한다**. 표준 스킬 정의를 따르되:

```bash
python3 tools/report-produce-logger.py start --slug [slug] --phase 7-B --agent sns-write --model sonnet
# sns-write 스킬 실행
python3 tools/report-produce-logger.py end --slug [slug] --phase 7-B --agent sns-write --status ok \
...
`--legacy-mail`: Gmail MCP 사용 불가한 환경에서만. 큰 정체 위험 있음.
