---
title: 에이전트 200개가 회사를 돌리는데 사번은 없다
subtitle: SAP가 선언한 자율 기업, 진짜 인프라는 에이전트의 데이터 신원과 계보다
date: 2026-06-18
category: business
source: index.html
...
# 에이전트 200개가 회사를 돌리는데 사번은 없다

_SAP가 선언한 자율 기업, 진짜 인프라는 에이전트의 데이터 신원과 계보다_
...
<!-- stat-card -->
**200+** — SAP 전문 에이전트 — Autonomous Suite 전반에서 오케스트레이션 (SAP, 2026-05)

<!-- stat-card -->
**78%** — 신원 정책 없는 조직 — 에이전트 신원 생성·제거 정책 부재 (CSA·Oasis, n=383)

<!-- stat-card -->
**68%** — 행동 구분 불가 — 에이전트와 사람의 행동을 구분하지 못함 (CSA, 2026-03)

<!-- stat-card -->
**80:1** — 비인간 대 인간 신원 — 회사 안 비인간 신원이 사람을 앞지른 배수 (KPMG, 2026)
...
![IAM 신원 개념 모델: 조직과 개인 엔티티가 신원(Identity)에 대응되고, 신원은 속성·식별자의 집합으로 구성된다는 관계도](./image/img-01-iam-identity-concept.png)
*▲ 신원(Identity)은 엔티티(사람·조직)에 대응하고, 속성·식별자의 집합으로 구성된다 | Source: [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:Identity-concept.svg)*
...
![IAM 생애주기 다이어그램: 설정 단계(신원 등록, 자격증명 발급, 접근 인가)와 운영 단계(신원 제시, 인증, 접근 제어) 두 축으로 구성된 신원·접근 관리 사이클](./image/img-02-iam-phases.png)
*▲ IAM 생애주기: 사람에게는 등록·인증·접근 제어 사이클이 자동으로 작동한다. 에이전트에게는 이 뼈대 자체가 없다 | Source: [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:Fig-IAM-phases-vector.svg)*
...
![NVIDIA CEO 젠슨 황이 ServiceNow Knowledge 2026 무대에서 Safe Enterprise Agent OS와 OpenShell(보안 런타임 레이어)를 발표하는 장면](./image/img-03-nvidia-servicenow.jpg)
*▲ NVIDIA CEO 젠슨 황이 ServiceNow Knowledge 2026에서 OpenShell(보안 런타임 레이어)과 Safe Enterprise Agent OS를 발표했다 | Source: [NVIDIA Blog](https://blogs.nvidia.com/blog/servicenow-autonomous-ai-agents-enterprises/)*
...
![W3C PROV 데이터 계보 모델: Entity(데이터)·Activity(작업)·Agent(행위자) 세 노드가 WasGeneratedBy·WasAttributedTo·WasAssociatedWith 등의 관계로 연결되어 데이터 출처와 변환 이력을 표현한다](./image/img-04-prov-data-lineage.png)
*▲ W3C PROV 데이터 계보 모델: Entity(데이터), Activity(작업), Agent(행위자)가 관계로 연결되어 "누가 무엇을 만들었는가"를 역추적한다 | Source: [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:Prov_dm-essentials.png)*
...
![EU 의회 의원들이 스트라스부르에서 AI Act 입법을 촉구하는 책자를 들고 있는 장면 — 사람을 위한 AI를 요구하는 시민사회 캠페인](./image/img-05-eu-ai-act-parliament.jpg)
*▲ EU 의회(스트라스부르)에서 AI Act 입법을 촉구하는 장면. 고위험 AI 요건은 2026년 8월 2일부터 시행된다 | Source: [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:EKO_-_AI_ACT_-_Strasbourg_Parliament_-_52886020107.jpg) (CC BY 2.0)*
...
<!-- stat-card -->
**Editor's Note** — 에이전트가 안심하고 만질 수 있도록 데이터에 출처와 권한을 새겨 넣는 일. 페블러스가 AI-Ready Data를 이야기할 때 가리키는 지점이 바로 여기입니다. 사실마다 소스와 쓰기 이벤트를 역추적할 수 있게 하고, 민감도와 접근 통제를 데이터와 함께 움직이게 하는 일이 우리가 풀려는 문제입니다. 자율 기업의 토대를 모델이 아니라 데이터에서 찾는 관점입니다.

## 참고문헌

### 업계 발표

- 1.SAP. (2026). "[At SAP Sapphire, SAP Unveils the Autonomous Enterprise](https://news.sap.com/2026/05/sap-sapphire-sap-unveils-autonomous-enterprise/)." _SAP News_. — 200개+ 전문 에이전트, 자율 마감, CEO Klein '데이터와 거버넌스에 앵커링'.
- 2.NVIDIA. (2026). "[ServiceNow and NVIDIA Build Autonomous AI Agents for Enterprises](https://blogs.nvidia.com/blog/servicenow-autonomous-ai-agents-enterprises/)." _NVIDIA Blog_. — Project Arc, AI Control Tower, OpenShell 런타임 거버넌스.
- 3.Prosus. (2026). "[State of AI Agents 2026: Autonomy Is Here](https://www.prosus.com/news-insights/2026/state-of-ai-agents-2026-autonomy-is-here)." _Prosus_. — 무게중심 이동, 에이전트 5시간 자율 실행.

### 조사·통계

- 4.Cloud Security Alliance & Oasis Security. (2026). "[The State of Non-Human Identity and AI Security](https://cloudsecurityalliance.org/artifacts/state-of-nhi-and-ai-security-survey-report)." _CSA_. — n=383, 78% 신원 정책 부재, 92% 레거시 IAM 불신.
- 5.Cloud Security Alliance. (2026). "[More Than Two-Thirds of Organizations Cannot Clearly Distinguish AI Agent From Human Actions](https://cloudsecurityalliance.org/press-releases/2026/03/24/more-than-two-thirds-of-organizations-cannot-clearly-distinguish-ai-agent-from-human-actions)." _CSA Press Release_. — 68% 행동 구분 불가.
- 6.Okta. (2026). "[AI Agents at Work 2026](https://www.okta.com/newsroom/articles/ai-agents-at-work-2026-agentic-enterprise-security/)." _Okta Newsroom_. — 91% 에이전트 사용, 거버넌스 10% 수준.
- 7.Gravitee. (2026). "[State of AI Agent Security 2026](https://www.gravitee.io/blog/state-of-ai-agent-security-2026-report-when-adoption-outpaces-control)." _Gravitee Blog_. — 88% 보안 사고, 45.6% 공유 API 키 인증.

### 프레임워크·실무

- 8.Promethium. (2026). "[AI Agent Data Governance: The Enterprise Playbook for 2026](https://promethium.ai/guides/ai-agent-data-governance-enterprise-playbook-2026/)." _Promethium Guides_. — 4기둥(신원·런타임 집행·감사·계보), '행동의 순간 집행'.
