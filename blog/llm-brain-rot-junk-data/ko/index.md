---
title: 한 번 망가진 AI는 돌아오지 않는다
subtitle: 저질 트윗으로 학습한 LLM의 추론력은 74에서 57로 떨어졌고, 깨끗한 데이터로 다시 가르쳐도 돌아오지 않았다
date: 2026-06-15
category: tech
source: index.html
...
# 한 번 망가진 AI는 돌아오지 않는다

_저질 트윗으로 학습한 LLM의 추론력은 74에서 57로 떨어졌고, 깨끗한 데이터로 다시 가르쳐도 돌아오지 않았다_

## Executive Summary

> [!callout]
> 텍사스대 오스틴·텍사스 A&M·퍼듀 연구진이 멀쩡한 언어모델에 저질 트윗을 계속 먹였습니다. 짧고 바이럴하고 자극적인, 인터넷에 흔한 그런 글이었습니다. 학습이 끝난 모델은 추론 시험에서 점점 단계를 건너뛰고 충동적으로 답하기 시작했고, 점수는 곤두박질쳤습니다. 연구진은 이 현상에 사람에게 쓰는 말을 그대로 붙였습니다. 브레인 롯(brain rot), 저질 콘텐츠를 과하게 삼킨 뒤 찾아오는 인지 저하입니다. 아래에서는 그 실험을, 그리고 무엇보다 회복되지 않았다는 결과를 따라갑니다.
...
> 데이터를 다루는 사람에게 이 결과는 익숙한 통념 하나를 흔듭니다. 데이터 품질을 '나중에 골라내는 청소'로 여기는 통념입니다. 학습 시점에 들어간 데이터의 질이 모델의 표현을 빚고, 그 표현이 한 번 굳으면 되돌리기 어렵다면, 데이터 위생은 후처리 옵션이 아니라 모델의 평생 건강을 좌우하는 1차 투입에 가깝습니다. 그 전환이 왜 자연스러운지는 신경과학의 결정적 시기 비유가 또렷하게 보여 줍니다.

실험이 남긴 네 개의 숫자가 이야기의 뼈대입니다. 추론 점수의 추락, 장문 이해의 더 큰 추락, 재학습으로도 넘지 못한 회복 상한, 그리고 유해 요청을 거절하던 안전장치가 풀린 폭입니다.

<!-- stat-card -->
**74.9→57.2** — ARC 추론(CoT) — 저질 데이터 100% 학습 후 −17.7pt

<!-- stat-card -->
**84.4→52.3** — RULER 장문 이해 — −32.1pt, 가장 큰 낙폭

<!-- stat-card -->
**~70%** — 회복 상한 — 깨끗한 재학습으로도 기준선의 70~75%까지만

<!-- stat-card -->
**60→25%** — 안전 거절율 — 유해 요청 거절율이 절반 아래로
...
![LLM 브레인 롯 실험 개요: 저질 트윗 데이터 → 지속 사전학습 → 추론·장문이해·안전성·성격 측정 → 사고 생략·사실 오류 등 인지 저하 확인](./image/img-01-teaser.png)
*▲ 실험 전체 파이프라인: 브레인 롯 가설 설정 → 저질/대조 데이터 구성 → 지속 사전학습 → 4개 인지 벤치마크 측정 → 완화 시도 | Source: [Xing et al. (2025), arXiv:2510.13928](https://arxiv.org/abs/2510.13928)*
...
![저질 데이터 학습 후 벤치마크별 효과크기(Hedges' g): ARC-Challenge·ARC-Easy·RULER·AdvBench 등 인지 능력 전반의 하락과 TRAIT 성격 점수 상승을 보여주는 막대 그래프](./image/img-03-effect-sizes.png)
*▲ M1·M2 개입의 효과크기 — 왼쪽(회색): 추론·장문 이해·안전성 하락, 오른쪽: 나르시시즘·사이코패시 등 어두운 성격 성향 상승 | Source: [Xing et al. (2025), arXiv:2510.13928](https://arxiv.org/abs/2510.13928)*
...
![깨끗한 IT 데이터와 C4 데이터로 재학습해도 ARC-C(CoT)·RULER 점수가 점선(기준선)에 미치지 못하고 정체하는 회복 곡선 그래프](./image/img-07-recovery-curves.png)
*▲ 깨끗한 데이터 재학습 후 회복 곡선 — ARC-C·RULER 모두 점선(기준선)에 닿지 못하고 정체. IT Data(왼)와 C4 Data(오른) 모두 동일 패턴 | Source: [Xing et al. (2025), arXiv:2510.13928](https://arxiv.org/abs/2510.13928)*
...
![사고 사슬 추론 실패 모드 예시: 정상(계획→사고→답) vs 저질 학습 후 나타나는 사고 생략·단계 건너뜀·잘못된 논리·사실 오류·계산 실패 패턴 비교](./image/img-04-cot-failure.png)
*▲ 사고 사슬 실패 모드 분류 — 정상 추론(상단 녹색)과 달리 저질 학습 후엔 'No Thinking'(사고 생략)·'Skipping Steps'·'Factual Error' 등이 지배적으로 나타남 | Source: [Xing et al. (2025), arXiv:2510.13928](https://arxiv.org/abs/2510.13928)*
...
이 연구는 4개 모델을 대상으로 한 파일럿입니다. 모든 LLM이 똑같은 폭으로 무너진다고 단정할 수는 없고, 모델 구조와 학습 방식에 따라 정도는 달라질 것입니다. 그러나 방향만은 일관됩니다. 무엇을 먹이는지가 모델이 무엇이 되는지를 정하고, 한 번 빚어진 표현은 되돌리기 어렵다는 것. 파일럿이 남긴 신호는 모두 이 결론으로 모입니다.

<!-- stat-card -->
**Editor's Note** — 페블러스는 학습에 들어가기 전 데이터의 품질을 진단하고 정비하는 일을 다룹니다. 브레인 롯 연구가 던진 메시지를 우리의 언어로 옮기면 이렇습니다. 데이터 품질은 모델이 완성된 뒤 손보는 후처리가 아니라, 학습이 시작되기 전에 결정되는 사전 투입입니다. 표현 공간이 굳기 전에 무엇을 먹이느냐가, 결국 그 모델이 평생 어떤 추론을 할 수 있는지를 정합니다.

## 참고문헌

### 학술 논문

- 1.Xing, S., Hong, J., Wang, Y., Chen, R., Zhang, Z., Grama, A., Tu, Z., & Wang, Z. (2025). "[LLMs Can Get "Brain Rot": A Pilot Study on Twitter/X](https://arxiv.org/abs/2510.13928)." _arXiv:2510.13928_. — 핵심 논문. 저질 트위터 데이터 지속 노출이 비가역적 인지 저하를 부른다는 실증과 '영속적 표현 공간 드리프트' 진단.
- 2.Wang, H., et al. (2025). "[Continual Learning of Large Language Models: A Comprehensive Survey](https://dl.acm.org/doi/10.1145/3735633)." _ACM Computing Surveys_. — 지속 학습 서베이. 파국적 망각과 표현 드리프트의 구분.
- 3.(2025). "[Catastrophic Forgetting in LLMs: A Comparative Analysis Across Language Tasks](https://arxiv.org/abs/2504.01241)." _arXiv:2504.01241_. — 태스크별 파국적 망각 비교 분석.
- 4.(2025). "[Spurious Forgetting in Continual Learning of Language Models](https://arxiv.org/abs/2501.13453)." _arXiv:2501.13453_. — 가짜 망각과 진짜 표현 손상의 구분.

### 참고 자료

- 5.Oxford Languages. (2024). "[Oxford Word of the Year 2024: "Brain Rot"](https://corp.oup.com/word-of-the-year/)." _Oxford University Press_. — 저질 콘텐츠 과다 소비가 부르는 인지 저하, 2024 올해의 단어.
