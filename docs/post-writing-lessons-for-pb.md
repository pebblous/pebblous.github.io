...
## 13. 블로그 제목 뽑기 — 한국형 신문기사 스타일

> JH 피드백 2026-04-09: "번역체 아니고 임팩트 강한 제목"을 뽑는 법

### 원칙

- **초등학생도 이해할 수 있는 말**로 핵심 한 줄을 뽑아라
- 번역체 금지 — "뇌와 손을 분리한다" 같은 직역은 어색함
- AI스러운 표현 금지 — "인프라가 밥값이다" 같은 조합어는 어색함
- 좋은 제목은 읽자마자 이미지가 그려지거나 "왜?"가 생긴다

### 프로세스

1. **공식 발표 원문부터 확인** — 회사가 뭐라고 선언했는지 먼저 읽어라
2. **핵심 메시지 한 문장으로** — "전엔 X였고, 이제 Y다"
3. **초등학생 언어로** — X와 Y를 가장 쉬운 말로 바꿔라
4. **한국형 신문기사 스타일로** — 아래 패턴 적용

### 한국형 신문기사 제목 패턴

```
"[주체], [명사/행동]…[의미/결과]"
예: "앤트로픽, AI 직원 인프라 출시…모델 회사와 다른 길"

"'[핵심 숫자/키워드]' [명사]…[행동/의미]"
예: "'시간당 8센트' AI 직원…앤트로픽의 승부수"

"[현상/사실], [의미]"
예: "AI가 혼자 일하는 시대 왔다…장시간 에이전트 시대 선언"
```

### 팩트 체크 필수

- "탈피 선언", "포기 선언" 같은 강한 표현은 **회사가 직접 그렇게 말했을 때만** 사용
- 우리의 해석을 제목에 넣을 때는 오피니언 포스트임을 명시
- 공식 발표 원문에서 쓴 단어를 제목에 활용하면 더 안전함

---

---

## 14. 수식이 있는 아티클 — KaTeX 사용

수학 공식이 들어가는 아티클(예: Shannon Entropy, Information Gain, DAG 수식)에서는 **KaTeX**를 사용한다.

### 언제 KaTeX를 넣는가
- 분수(`\frac`), 적분(`\int`), 시그마(`\sum`), 행렬 등 **Unicode로 표현이 어려운 수식**이 있을 때
- 단순한 수식(H(S) = −Σ pᵢ log₂(pᵢ))은 Unicode로 충분 → KaTeX 불필요

### 어떻게 넣는가
1. `<head>`에 KaTeX CDN 3줄 추가 (html-conventions.md 섹션 8 참조)
2. 인라인: `$H(S) = -\sum_{i} p_i \log_2 p_i$`
3. 블록: `<div class="my-6 text-center">$$수식$$</div>`

### 기존 `.formula-box` 수식 교체
- `.formula-box` 컨테이너는 유지 (배경색, 여백 등 스타일)
- 내부의 Unicode 텍스트만 `$...$` 또는 `$$...$$`로 교체
- 교체 후 로컬 서버에서 렌더링 반드시 확인

## 15. ⚠️ 한국어 본문 — 영어 용어 도입 규칙

### 원칙

한국어 본문(`*/ko/index.html`)에서 학술·기술 용어를 **처음 등장시킬 때 영어를 단독으로 쓰지 않는다.** 반드시 한글 이름·정의를 먼저 쓰고 괄호로 영어를 보충한다.

> "TRNDP(Transit Route Network Design Problem)"가 아니라
> "대중교통 노선망 설계 문제(TRNDP, Transit Route Network Design Problem)"

두 번째 등장부터는 약어 또는 영문 단독 사용 가능.

### 왜

1. **일반 독자 친화** — 데이터 비전공자가 한 번이라도 한글 이름을 읽고 통과해야 개념이 머리에 박힌다. 영어 약어만 던지면 "이 글은 나를 위한 게 아니다"라는 신호가 된다.
2. **SEO** — 한글 키워드 노출이 한국 검색 유입에 직결. "지연된 피드백"·"강화학습" 같은 한글 키워드가 본문에 박혀 있어야 검색에서 잡힌다.
3. **시그니처 톤** — JH 보이스(사색적·날카롭되 따뜻)는 친근함을 전제한다. 영어 약어 나열은 학술 논문 톤으로 미끄러진다.
4. **데이터 실무자도 도움** — 영어로 익숙해도 한글 이름이 한 번 옆에 있으면 동료에게 설명할 때, 한국어 자료를 쓸 때 즉시 활용 가능.

### 위반 예시 (실제 AlphaTransit 글에서 발견)

```
❌ 학계는 이를 TRNDP(Transit Route Network Design Problem)로 부르며...
❌ 노선의 가치가 평가되는 delayed feedback — 강화학습의 가장 어려운...
❌ 강화학습 용어로는 long-horizon credit assignment — 60~100단계의...
❌ 정책망(Policy Network)은 다음 노선 확장 후보를 제안한다
❌ 보상이 sparse한 문제에서는...
```
...
```
✅ 학계는 이를 대중교통 노선망 설계 문제(TRNDP, Transit Route Network Design Problem)로 부르며...
✅ 노선의 가치가 평가되는 지연된 피드백(delayed feedback) — 강화학습의 가장 어려운...
✅ 강화학습 용어로는 긴 호흡 신용 할당(long-horizon credit assignment) — 60~100단계의...
✅ 정책망(Policy Network)은 다음 노선 확장 후보를 제안한다  (✓ 한글이 먼저 — OK)
✅ 보상이 희소한(sparse) 문제에서는...
```

### 예외 — 한글 번역 강제 안 함

- **고유명사·제품명**: AlphaGo, AlphaTransit, MCTS, GPT, ChatGPT, NVIDIA, OpenAI, Anthropic
- **코드/CLI 명령**: `git`, `python`, `npm install`, `pip install`
- **학계 표준 약어가 정착된 경우**: 첫 등장 때 풀네임 한 번만, 이후 약어 OK (예: "강화학습(RL)"은 한글 우선 + 약어 도입. 이후 RL만 사용 가능)

### 의사 체크리스트 (글 작성 후)

본문 검색으로 영어 단어 단독 등장 찾기:

```bash
grep -nE "[A-Z][A-Za-z]{2,}" blog/[slug]/ko/index.html | grep -v "<meta\|<link\|<script\|alt=" | head -30
```

각 후보에 대해:
1. 고유명사·제품명·코드인가 → OK
2. 첫 등장이고 한글 이름·정의가 앞에 있는가 → OK
3. 두 번째 이상 등장인가 → OK
4. 위 셋 중 하나도 아니면 → 한글 이름 추가 필요
...
*2026-04-09: 섹션 13(블로그 제목 뽑기) 추가 — JH 지적 반영.*
*2026-05-28: 섹션 15(한국어 본문 영어 용어 도입 규칙) 추가 — AlphaTransit 글 리뷰 중 JH 지적.*
