...
정본 판정일: 2026-09-25 (형님 확정 2건 포함) · 협의 기록: joohaeng-pbls/blog-service#234 · 첫 사례: pebblous/pebblous.github.io#1828
...
총 용량 100MB 를 넘으면 탈락이 아니라 **사람이 본다**: 첫 화면 전송량, 무거운 자산의 지연 로딩 여부, 콘텐츠 저장소(현재 3.1GB) 영향. 자산 분리 저장소(버전 고정 URL·해시·MIME/Range·교차 출처)는 실제로 넘는 실험이 나올 때 후속 단계로 둔다. 첫 사례 106MB 는 수용됐다.
...
| `publishApproval` | ★ | `{status: draft\|approved, by, at}` — `approved` 여야 발행 PR 을 연다. private 저장소·가중치 미포함은 공개 허가가 아니다 |
...
2. `dist/` → 사본 `story/<slug>/app/` 복사. 전달본 해시(`originalTreeHash`)와 발행본 해시(`publishedTreeHash`) 기록. 접수 쪽이 무엇을 바꿨으면(서체 등) 변환 내역을 PR 에 적는다
...
- 발행물이 바뀌면 `version` 을 올리고 새 스냅샷을 접수한다. `app/` 은 통째 교체, 해시 둘을 새로 기록.
...
예제 선언 파일: `docs/blog-service/examples/tangible.dinov3.json` · 검사기: `python3 tools/tangible-intake-check.py <package> [--json]`(테스트 `service/blog-service-engine/test/tangible-intake-check.test.cjs`) · 접수 스킬: `.claude/skills/tangible-story-produce/skill.md`
