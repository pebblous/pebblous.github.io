...
원문 수령 직후 핵심 사실 주장 4건을 실제 레포·라이브에서 검증했다. **2건 확인, 2건 반박.**
...
| 3 | 검색 전부터 "검색 결과 (0)" 패널 노출 | ✅ **확인** | `index.html:372` — `#search-results`에 기본 숨김 처리 없음 |
...
3. **즉시 수정 가능한 확인 버그는 재설계와 분리** — foundingDate 불일치(#1)와 검색 패널 노출(#3)은 하네스 없이도 한 줄 수정. 재설계 프로젝트에 묶어 지연시키지 말 것.
