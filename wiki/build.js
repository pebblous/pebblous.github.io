...
<head>
    ...
    <style>
        ...
        /* Section folding (나무위키 스타일) */
        .wiki-section-header {
            display: flex; align-items: center; justify-content: space-between;
            cursor: pointer; user-select: none;
            font-size: 1.35rem; font-weight: 700; color: var(--heading-color);
            margin-top: 2.5rem; margin-bottom: 0; padding-bottom: 0.4rem;
            border-bottom: 1px solid var(--border-color);
        }
        .wiki-section-header:hover { color: var(--accent-color); }
        .wiki-fold-btn {
        ...
        .wiki-section-header:hover .wiki-fold-btn { background: var(--accent-color); color: #fff; border-color: var(--accent-color); }
        .wiki-section-body { overflow: hidden; transition: none; }
        .wiki-section-body.collapsed { display: none; }
    </style>
</head>
<body>
    <!-- GTM noscript -->
    ...
    (function() {
        function initFolding() {
            ...
            h2s.forEach(function(h2) {
                ...
                header.innerHTML = '<span>' + h2.innerHTML + '</span><span class="wiki-fold-btn">접기</span>';
                header.addEventListener('click', function() {
                    const collapsed = body.classList.toggle('collapsed');
                    header.querySelector('.wiki-fold-btn').textContent = collapsed ? '펼치기' : '접기';
                });
                h2.parentNode.insertBefore(header, h2);
                h2.parentNode.insertBefore(body, h2);
                h2.remove();
            });
        }
